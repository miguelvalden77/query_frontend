...
const PersonalDescription = ({ userId, getData, info }: propsPersonalInfo) => {
    ...
    return <section className="description-section">
        {
            click ? <textarea onChange={handleChange} value={personalInfo} name="personalDescription" ></textarea> : <p>{personalInfo ? personalInfo : "No hay estado aún"}</p>
        }
        {userId == usuario?.id && <button style={click ? { borderRadius: "0px 3px 3px 0px" } : { borderRadius: "3px" }} onClick={handleClick}>Actualizar</button>}
    </section>
